...
import BaseHTTPServer, time, random, argparse, subprocess, os, stat, itertools 

HOST_NAME = "127.0.0.1"
PORT = 9147
PLAYER_NAME = "roblaing"
PROLOG = '#!/usr/bin/yap -L --'
# PROLOG = '#!/usr/bin/swipl -f -q'
PROLOG_FILE_NAME = PLAYER_NAME + str(os.getpid()) + ".pl"
DOT_FILE_NAME = False
...
def create_prolog_file(rules, extra_rules):
    """
    This generates a prolog script which either yap or swipl can run.
    The default is yap. To use swipl, change global constant PROLOG to
    PROLOG = '#!/usr/bin/swipl -f -q'
    """
    ...
    def or_rewrite(rule):
        return '(' + ';'.join([body[0] + str(body[1:]).replace('[', '(').replace(']',')').replace("'",'') \
            for body in rule[1:]]) + ')'
        
    out_file = open(PROLOG_FILE_NAME, 'w')
    print(PROLOG, file = out_file)
    print(':- set_prolog_flag(verbose, silent).', file = out_file)
    print(':- initialization(main).', file = out_file)
    print('distinct(A, B) :- A \= B.', file = out_file)
    for rule in rules:
        if rule[0] != '<=':
            print(rewrite(rule) + '.', file = out_file)
        else:
            print(rewrite(rule[1]) + ' :- ' + ", ".join([rewrite(body) \
              for body in rule[2:]]) + '.', file = out_file)
    for extra_rule in extra_rules:
        print(extra_rule, file = out_file)
    out_file.close()
    st = os.stat(PROLOG_FILE_NAME)
    os.chmod(PROLOG_FILE_NAME, st.st_mode | stat.S_IEXEC)
    
def run_prolog_file():
    """
    This uses subprocess.check_output(...) to run the prolog script generated by create_prolog_file(...).
    If the result is an atom (as for terminal which returns "True" or "False"), it returns that.
    Usually the result is a list of terms, often with many duplicates. Instead of handling that 
    in prolog (beyond my skill at time of writing), python's set is used to remove duplicates and 
    then this gets sorted into a tuple which can be used as a key for the game dictionary. 
    """
    ret_str = subprocess.check_output('./' + PROLOG_FILE_NAME)
    if ret_str.find('[') == -1:
        return ret_str
...
def findmoves(state, game):
    ...
    if 'actions' not in game['tree'][state]:
        ...
        extra_rules = []
        for prop in state:
            extra_rules.append('true(' + prop + ').')
        extra_rules.append('main :- findall([R,M], legal(R,M), L), write(L), halt.')
        create_prolog_file(game['rules'], extra_rules)
        legals = run_prolog_file()
        for legal in legals:
            idx = legal.index(',')
...
def findnext(moves, state, game):
    ...
    if 'next' not in game['tree'][state]['actions'][moves]:
        ...
        for idx in range(len(moves)):
            if moves[idx] != 'noop':
                extra_rules.append('does(' + roles[idx] + ',' + moves[idx] + ').')
        for prop in state:
            extra_rules.append('true(' + prop + ').')
        extra_rules.append('main :- findall([B], next(B), L), write(L), halt.')
        create_prolog_file(game['rules'], extra_rules)
        game['tree'][state]['actions'][moves]['next'] = run_prolog_file()
...
def findreward(role, state, game):
    ...
    if 'values' not in game['tree'][state]:
        ...
        for prop in state:
            extra_rules.append('true(' + prop + ').')
        extra_rules.append('main :- findall([Role, N], goal(Role, N), L), write(L), halt.')
        create_prolog_file(game['rules'], extra_rules)
        rewards = run_prolog_file()
...
def findterminalp(state, game):
    ...
    if 'terminal' not in game['tree'][state]:
        ...
        for prop in state:
            extra_rules.append('true(' + prop + ').')
        extra_rules.append("end :- terminal, write('True').")
        extra_rules.append("end :- \+terminal, write('False').")
        extra_rules.append('main :- end, halt.')
        create_prolog_file(game['rules'], extra_rules)
        game['tree'][state]['terminal'] = (run_prolog_file() == 'True')            
